 ...
 *
 * Imprime el nombre del archivo de entrada dado y la distancia
...
fun main(args: Array<String>) {
    ...
    val tourOpt = busquedaLocalCon2Opt(tour)
    val distOpt = calcularDistanciaTour(tourOpt) 
    println("TSP instance: ${args[0]}\n")
    println("Total distance: $dist\n")
    println("Total distance Opt: $distOpt\n")
 ...
 * ciudades.
 */
fun swap(P: CityArray, i: Int, j: Int) {
    val temp = P[i]
    P[i] = P[j]
    P[j] = temp
}
 ...
 */
fun insertionSort(P: CityArray, eje: Char = 'X') {
    val n = P.size
...
fun particion(P: CityArray, a: Int, b: Int, eje: Char = 'X'): Int {
    ...
    while (true) {
        ...
        if (i < j) swap(P, i, j) else return i
    }
}
 ...
 * Las comparaciones se hacen con respecto al [eje].
 */
fun quickSortCiclo(P: CityArray, a: Int, b: Int, profundidadMaxima: Int, eje: Char = 'X') {
    var r = b
    ...
    while (r - a > 16) {
        ...
        quickSortCiclo(P, p, r, profundidadMaxima - 1, eje)
        r = p
    }
}
